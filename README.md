...
```bash
$ git clone https://github.com/PretendoNetwork/f1-2011
$ cd f1-2011
```
...
```bash
$ docker build -t f1 --build-arg BUILD_STRING=YOUR_BUILD_STRING_HERE .
...
Example:
```
PN_F1_AUTHENTICATION_SERVER_PORT=61001
PN_F1_SECURE_SERVER_PORT=61002
...
```

Then, you can use the following command to run the image.
```bash
$ docker run --name f1 --env-file .env -it f1
```
...
$ go get -u
$ go mod tidy
$ go build -o build/f1-2011
```

The server is now built to `build/f1-2011`
...
The server is now built to `build/f1-2011` with the authentication server build string already set
